...
- **Reactive agent cwd (worktree shifting).** Pi binds cwd at session creation and historically made mid-session cwd changes painful — no easy "switch to a worktree and back." UIX owns session construction, so it can expose the maneuver: `SessionManager.open(path, sessionDir, cwdOverride)` re-opens the _same_ session file under a different cwd, and an explicitly pinned `sessionDir` keeps the session file and `.uix` canvases on the stable state root while only the agent cwd moves. Implication for the two-tier session ([persistence-and-session-foundation](../plans/persistence-and-session-foundation.md) C0): keep the agent cwd a single mutable seam — dispose/recreate the lazy agent tier on change and emit a refresh for cwd-derived surfaces (coding tools, project-local discovery) — while canvas/anchor state, being cwd-independent, never moves. Not built yet (single-agent, no concurrent sessions assumed); the C0 state-root/agent-cwd split is the only pre-payment. Settle when an app actually needs the shift.
