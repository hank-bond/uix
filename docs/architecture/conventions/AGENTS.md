...
- **[contributing](./contributing.md)** _(how-to)._ Define conventions in their owning documents, apply them within the agreed scope, and track deferred migrations separately. _Read before proposing changes to any conventions document._
- **[injected-capabilities](./injected-capabilities.md)** _(reference)._ Injected capabilities serve current UIX production boundaries instead of creating test-only indirection or provider lookalikes. _Read before adding or changing injected callbacks, dependency objects, context capabilities, adapters, factories, or their test doubles._
- **[lexicon/](./lexicon/AGENTS.md)** The controlled lexicon assigns every UIX word one meaning and grammatical role, split by reader task into identifier, prose, and imported terms. _Read before introducing a recurring word, reviewing wording in comments or docs, or extending the controlled lexicon._
- **[lifetimes](./lifetimes.md)** _(reference)._ Disposable lifetimes own cleanup. Guards prevent disposal of a thing or the start of an action, while tracked operations own cancellation and completion. _Read before attaching callbacks, listeners, IPC, protocols, timers, or any other behavior that requires cleanup._
