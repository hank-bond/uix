...
Few modules need direct `process` access. Environment, working-directory, and platform reads belong in the Electron host (`src/main/index.ts`) or the runtime utilities it composes (`packages/runtime/src`). Feature code does not import `process` directly. Add a runtime environment capability to `FeatureContext` when a feature needs one.
