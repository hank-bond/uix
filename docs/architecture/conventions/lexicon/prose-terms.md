...
| `canonical URL` | noun | The workspace-session deep link (`/workspaces/:workspaceId/sessions/:sessionId`) that is one attachment's authoritative target. | "the canonical URL names the target" | "the selected session URL" |
...
| `discovery` | noun | The look-up of an active server's advertised address. | "server discovery", "a native launcher discovers the server" | "the server registry" |
| `fallback session` | noun | The newest valid session, or a new session, resolved only for an explicit fallback attachment admission. Electron uses this policy for its initial workspace window. The web host instead creates a fresh session for a workspace-only address. | "Electron admits its initial attachment through the fallback session policy" | "the web workspace-only route resolves the fallback session" |
| `guard` | noun | Disposable protection named for what it prevents: disposal of a thing or the start of an action. Disposing a guard removes only that holder's protection. It does not perform the prevented action. The naming forms live in the [role-terms section](./code-terms.md#uix-owned-role-terms). | "the run holds an agent instance guard against disposal and a reload guard against reload" | "reload holds a reload guard" when that capability permits reload rather than preventing it |
...
| `namespace` | noun | A group of local names under one owner. It prevents those names from colliding with names from other owners. A namespace does not choose a Workspace, viewpoint, or live target. | "the `canvas` channel namespace", "the `uix` web namespace" | "the namespace selects the current Agent" |
| `prepared dispatch` | noun | One accepted canonical request with immutable attachment authority, a resolved workspace channel entry, and an agent instance guard. The host uses its contract policy to log before invoking it. | "the prepared dispatch survives attachment retarget" | "the attachment's log policy" |
...
| `run` | noun | One complete exchange between a user prompt and the matching agent response. A run contains one or more turns, each covering one model response and its tool executions. | "One prompt starts a run containing three model turns." | "Each model response starts another run within the same prompt exchange." (use `turn`) |
| `single-flight` | adjective | Concurrent identical requests share one in-flight operation promise, so the underlying work executes once. | "single-flight instance creation" | "coalesced instance creation" |
...
| `turn` | noun | One model response plus its tool executions within an agent run, not the complete prompt exchange. | "Checkpoint after each model turn within the run." | "One turn contains three model responses and their tool executions." (use `run`) |
...
| `deliver` | verb | Provide one already validated and scoped event to one receiver selected by its event scope. Do not use for requests or physical transport messages. The code vocabulary lives in the [operation-terms section](./code-terms.md#uix-owned-operation-terms). | "the attachment receives the delivered event" | "the registry delivers the canonical request" |
| `dispatch` | verb | Route an accepted, host-neutral request using its established attachment context toward its registered handler. Do not use for event publication, event delivery, physical transport, or the handler's later domain effects. The code vocabulary lives in the [operation-terms section](./code-terms.md#uix-owned-operation-terms). | "the attachment dispatches the channel request", "the runtime dispatches the viewpoint web request" | "the host dispatches the agent event" |
...
| `retain` | verb | Hold onto a value or membership across changes. On a guard, retain means mint another independently disposable guard for the same supervised object. Prefer over `keep` for this sense. | "retains the latest value", "dispatch retains an agent instance guard from the attachment's target guard" | "keeps the latest value" |
...
| `send` | verb | Write one encoded request, response, or event message through a physical connection. Do not use for semantic request routing or event delivery. The code vocabulary lives in the [operation-terms section](./code-terms.md#uix-owned-operation-terms). | "the host sends the response message" | "the workspace sends the request to its handler" |
