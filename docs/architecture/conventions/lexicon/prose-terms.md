...
| `guard` | noun | Disposable capability whose existence prevents a supervisor from tearing down one shared live object. Every holder receives an independently releasable guard. Releasing a guard removes that holder's protection but does not promise teardown. A guard may provide access to the protected object or a separate handle, where ordinary operations remain. | "the turn holds an agent instance guard", "the connection owns a workspace guard" | "the attachment guard dispatches the request" |
...
| `release` | verb | Relinquish one guard or explicitly held capability. Releasing a guard removes only that holder's teardown protection and does not promise teardown of the guarded object. | "the attachment releases its previous guard" | "releases the agent" for tearing down the instance |
