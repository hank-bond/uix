...
| `agent instance` | noun | Lifecycle owner for one Pi execution at a session-branch viewpoint. It owns its private session manager and restored state before its Pi runtime boots. | "the primary agent instance", "an agent instance boots" | "the agent runtime owns the session manager before it boots" (use `agent instance`) |
...
| `guard` | noun | Disposable capability whose existence prevents a supervisor from starting teardown for one shared live object. Every holder receives an independently releasable guard. Releasing a guard removes that holder's protection but does not promise teardown. A guard may provide access to the protected object or a separate handle, where ordinary operations remain. | "the turn holds an agent instance guard", "the connection owns a workspace guard" | "the attachment guard dispatches the request" |
...
| `supervisor` | noun | Lifecycle owner for keyed shared live children. It owns child identity, single-flight boot, independent guard admission, lifetime policy, and teardown, then provides the protected child or narrower handles for ordinary work. | "the workspace supervisor", "the agent instance supervisor" | "the workspace router" for connection traffic |
| `teardown` | noun | Owner-coordinated lifecycle process that ends one supervised shared child's lifecycle after policy admits it. Use `dispose` for the deterministic cleanup operation on one object. | "the supervisor starts instance teardown after every guard releases" | "teardown the instance" (use "dispose the instance" or "start instance teardown") |
...
| `dispose` | verb | Invoke one object's deterministic cleanup operation. Use `release` for one guard and `teardown` for the owner-coordinated supervised lifecycle process. The code vocabulary lives in the [operation-terms section](./code-terms.md#uix-owned-operation-terms). | "the supervisor disposes the instance during teardown" | "the attachment disposes the shared instance when it closes" (it releases its guard) |
...
| `release` | verb | Relinquish one guard or explicitly held capability. Releasing a guard removes only that holder's teardown protection and does not promise teardown of the guarded object. | "the attachment releases its previous guard" | "releases the agent" for disposing the instance |
...
| `retain` | verb | Hold onto a value or membership across changes. On a guard, retain means mint another independently releasable guard for the same supervised object. Prefer over `keep` for this sense. | "retains the latest value", "dispatch retains an operation guard from the attachment guard" | "keeps the latest value" |
