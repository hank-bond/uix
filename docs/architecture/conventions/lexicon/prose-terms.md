...
| `guard` | noun | Disposable protection named for what it prevents: disposal of a thing or the start of an action. Disposing a guard removes only that holder's protection. It does not perform the prevented action. The naming forms live in the [role-terms section](./code-terms.md#uix-owned-role-terms). | "the turn holds an agent instance guard against disposal and a reload guard against reload" | "reload holds a reload guard" when that capability permits reload rather than preventing it |
