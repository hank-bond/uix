...
| `Handle` | noun | Object that gives its holder a scoped capability. Use `Handler` for a callable that processes an occurrence. A `WorkspaceHandle` is the host-facing handle to one in-process workspace runtime. | `SettingsHandle`, `WorkspaceHandle` | `ChannelTransportHandle` for a registrar callable |
