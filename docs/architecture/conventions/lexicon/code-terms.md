...
| `Attachment` | noun | A connection's owned, retargetable handle on one agent instance. It is the routing identity for that connection's requests, can retarget to another instance, and disposes when the connection drops. Use `Handle` for a scoped capability with no live counterpart. | `ClientAttachment`, "the connection's attachment retargets" | `SessionBinding` |
...
| `Handle` | noun | Object that gives its holder a scoped capability. Use `Handler` for a callable that processes an occurrence. A `WorkspaceHandle` is the host-facing handle to one workspace runtime, in local or proxy form. | `SettingsHandle`, `WorkspaceHandle` | `ChannelTransportHandle` for a registrar callable |
...
| `Supervisor` | noun | Host-internal component that supervises and routes requests to workspace runtimes. Use `Coordinator` for sequencing a lifecycle across participants and `Driver` for ownership of one runtime boundary. | `WorkspaceSupervisor` | `WorkspacePool`, "the supervisor app" for a launcher |
...
| `boot` | verb | Provision a live agent instance for a target when none exists. A boot is a discrete provisioning step. Attaching to the booted instance is separate. Concurrent attaches share one boot through single-flight. | `bootAgentInstance()` | `attach` for provisioning |
