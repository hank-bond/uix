...
| `Guard` | noun | Disposable capability whose existence prevents a supervisor from tearing down one shared live object. Every holder receives an independently releasable guard. A guard can provide access to the protected object or a separate operational handle, but ordinary domain operations remain there. Releasing one or every guard removes protection without promising teardown because owner policy may keep the object live. | `AgentInstanceGuard`, `WorkspaceGuard` with a `WorkspaceHandle` | `AgentInstanceRetention`, `WorkspaceGuard` that directly owns workspace operations |
| `Handle` | noun | Object that gives its holder a scoped operational capability. Use `Guard` when the capability's defining purpose is preventing teardown, and `Handler` for a callable that processes an occurrence. A `WorkspaceHandle` is the narrow host-facing capability for one supervised workspace. It creates attachments but exposes no supervisor-owned teardown authority. | `SettingsHandle`, `WorkspaceHandle` | `AgentInstanceHandle` for a lifetime guard, `ChannelTransportHandle` for a registrar callable |
...
| `Supervisor` | noun | Lifecycle owner for a keyed set of shared live children. It owns child identity, single-flight boot, guard admission, lifetime policy, and teardown. Acquisition returns independent guards, while ordinary work uses the protected child or a narrower handle instead of traversing the supervisor. Use `Coordinator` for sequencing a lifecycle across independently owned participants and `Driver` for ownership of one external runtime boundary. | `WorkspaceSupervisor`, `AgentInstanceSupervisor` | `WorkspaceRouter` for the supervisor merely because it provides connection-bound handles |
...
| `attach` | verb | Establish a connection's owned handle on one agent instance, booting the instance first when none is live. Both the connection and the instance exist before the attach, which is a later step. Use `bind` only for creation-time wiring. | `attachment.attach(sessionBranch)` | `bind` a connection to an agent instance |
...
| `boot` | verb | Provision an agent instance for a target when none exists. Instance boot establishes its private session manager and restored state. Its Pi runtime may boot lazily later. Attaching to the booted instance is separate. Concurrent attaches share one boot through single-flight. | `bootAgentInstance()` | `attach` for provisioning |
...
| `release` | verb | Relinquish one guard or other explicitly held capability. Releasing a guard is immediate and removes only that holder's teardown protection. It does not promise teardown of the guarded object. | `agentInstanceGuard.release()` | `releaseAgentInstance()` for disposing the instance itself |
...
| `retain` | verb | Mint another independently releasable guard for the same supervised object from a live guard. Use `acquire` to resolve or boot by target and `release` to relinquish one guard. | `const operationGuard = attachmentGuard.retain()` | `guard.clone()` when the result establishes an independent lifetime claim |
...
| `teardown` | verb | End a live object's lifecycle at its safe boundary. Use `dispose` for the single deterministic cleanup call. | `teardownAgentInstance()` | `retireAgentInstance()` |
