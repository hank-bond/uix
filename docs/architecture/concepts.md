...
An `AgentInstanceSupervisor` owns the live instances for one workspace runtime. The current policy keys one primary instance by session id, single-flights concurrent creation, and issues independent _guards_. A guard prevents teardown while its holder uses the instance. Releasing one guard is synchronous and affects no peer. Zero guards admits supervisor teardown policy rather than promising teardown to the releaser.
