...
- **Workspace tools:** Provides exact-name reason-bearing `read`, `write`, `edit`, and `shell` tools. Each read captures its image type and bytes under Pi's shared file-mutation queue before Pi formats the result. Same-file tool mutations cannot interrupt that capture; unrelated files remain parallel. Shell commands and external writers do not participate.
