...
**In flight:** chat render primitives are being hardened into the shape that later becomes renderer/style/asset contributions; durable transcript identity is the next substrate needed before interactive or stateful chat blocks. Filesystem-tool parity and pane versioning/rollback are tracked in [persistence-and-session-foundation](../plans/persistence-and-session-foundation.md), [canvas-data-channel](../design/canvas-data-channel.md), and [pane-and-file-versioning](../design/pane-and-file-versioning.md).
