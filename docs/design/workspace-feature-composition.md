...
The next Agent composition step separates one workspace feature generation from the mutable Agent facets instantiated for each `AgentInstance`. H4 keeps one implicit Agent composition across the workspace, represented internally as an explicit composition over the reusable Agent-facet catalog rather than as an unmodeled “all facets” rule. Agent personalities, role selection, and agent-to-agent messaging remain deferred until after host extraction. The instance boundary should permit later workspace-manifest definitions to name compositions that select different tools, skills, prompt sections, context, and other Agent facets without adding those concepts to the current author or wire contracts.
...
The author contract remains flat. Each optional facet is an independently invoked factory, and the substrate privately classifies its workspace registration, Agent instantiation, and Pi installation operations. Features describe capabilities instead of scheduling. One feature entry remains the source, admission, and reload unit even when it contains only Agent facets or only workspace facets.

Workspace and Agent contexts are disjoint bumper lanes. Workspace context extensions reach only workspace facet factories. Each Agent instance receives a fresh feature context containing narrow viewpoint-scoped capabilities and no workspace context extension or mutable repository owner. Shared immutable definitions and substrate-owned repositories remain valid, while mutable Agent state never crosses viewpoints. Successful sibling facets remain live after a facet failure, and each operation retains structured internal outcome identity for later diagnostics.
