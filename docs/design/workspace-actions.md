...
The manifest shape should serve humans and agents rather than mirror the current fixed-key `SettingsRegistry`: `settings.keybindings` directly maps action ids to shortcuts or `null`, without a redundant inner `bindings` cell. Its dynamic property names remain schema-validated against the canonical action-id grammar. Well-formed ids that do not resolve against the active renderer registry are retained for feature reinstallation and included as unresolved diagnostics in the derived projection, so a future editor can distinguish malformed data from a possible typo or dormant choice.
