---
summary: "A connection attachment owns a guard on one session-bound agent instance and dispatches with guarded authority. Turns and asynchronous operations hold independent guards, and the instance supervisor applies teardown policy only after every guard disposes."
kind: explanation
status: resolved
---
...
The host resolves the workspace from the canonical connection URL, acquires a `WorkspaceGuard` through the workspace supervisor, and asks its operational `Workspace` value to create an attachment for the requested session. The runtime's `AgentInstanceSupervisor` owns the session-to-primary-instance policy, single-flight creation, guard admission, lifetime policy, and teardown. Concurrent attaches to an absent instance await one shared creation promise. Later attaches receive guards on the existing instance whether its Pi runtime is unbooted, idle, or running.

`attach` is the connection's binding step. When no instance exists for the target, the runtime creates one first, including its private manager and restored state. The connection then attaches to it. The instance boots its Pi runtime lazily only when an operation requires execution. `bind` remains creation-time wiring only and does not describe the connection-to-instance relationship.
...
The original attachment model was distilled into [`2026-08-09-attachments-target-agent-instances.md`](../decisions/2026-08-09-attachments-target-agent-instances.md). The guard-native refinement below replaces that record's retention-token mechanism without changing its attachment, single-flight creation, or shared-instance conclusions. Zero-guard idle periods, always-on, multi-agent, and ephemeral policies remain future work.
...
**Attach is separate from creation.** A connection attaches to an instance. The runtime creates one first when none exists, including its private manager and restored state. The instance supervisor owns both creation admission and target-guard acquisition, while the attachment remains the connection capability. Creating the instance does not boot its lazy Pi runtime. Tradeoff: attach can await creation even though disposing its later guard is synchronous.

**Acquire before disposal on session switch.** A failed target creation leaves the connection on its accepted old instance and URL. After acquisition, the attachment replaces its target guard and disposes the old guard immediately. Old operations continue under independent guards. Tradeoff: both instances can remain guarded briefly during the switch, and session-scoped event delivery must already prevent old activity from reaching the new target.
...
**Turns guard their own safe boundary.** A started turn derives a guard before detached work begins and disposes it after the final commit-safe boundary. Disconnecting disposes only the attachment guard, so the turn can finish with no observing client. The supervisor needs no guard operation that waits for `agent_end`; zero guards is already a safe teardown point. Final branch-scoped feature state commits at actual teardown rather than whenever one attachment leaves. Tradeoff: detached operations must dispose their guards reliably through `using` or their longer-lived holder's disposal protocol.
...
Moved attach, retarget, single-flight boot, and agent teardown into each workspace runtime's agent instance manager. The host resolves the workspace and owns the physical connection, while the runtime owns which shared agent the attachment reaches. Session switches acquire the target before disposing the old guard and update the URL only after success.
...
Replaced retention tokens and disposal-triggered safe-boundary waits with `AgentInstanceGuard` capabilities issued by an `AgentInstanceSupervisor`. Attachments own replaceable target guards. Prepared dispatch retains an immutable operation guard, and a started turn owns another guard through its final safe boundary. Disposing any guard is immediate and cannot promise instance disposal. Zero guards only admits supervisor teardown policy. This makes retarget non-blocking by construction and gives reload and future background work the same explicit lifetime rule.
...
### 2026-08-14: instance creation is separate from Pi runtime boot

Refined the earlier boot vocabulary. The supervisor single-flights agent instance creation, which establishes the manager and restored state before issuing a guard. `boot` now names only the later lazy start of that instance's Pi runtime. Creation needs no guard because the supervisor's in-flight promise owns the instance exclusively until admission; every consumer receives a guard before the instance escapes.
