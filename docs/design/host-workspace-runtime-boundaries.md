...
### 2026-08-15: physical connections bind attachments without payload routing
...
### 2026-08-27: Electron becomes a discrete supervised host

Moved Electron main, preload, renderer bootstraps, IPC contracts, native assets, and electron-vite configuration under `hosts/electron`. The host now composes the shared `WorkspaceSupervisor`. Every workspace `webContents` binding owns one guard and attachment, and canonical IPC dispatch resolves by that physical connection identity. Electron registers its privileged protocol once and selects runtime resource handlers through a host-wide workspace-qualified transport. Dependency enforcement now rejects Electron imports from every host-neutral package, the server host, and app features.
