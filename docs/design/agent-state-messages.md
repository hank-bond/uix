...
**Where extensions get pi (the principle behind all of this).** Raw pi reaches extensions in **exactly one place**, never sprinkled across wrapped concepts: an extension wanting raw agent capability declares a `pi` field and **pi loads it natively** (the driver's resource loader already discovers user pi resources alongside the core extension). `@uix/api` carries only **framework-mediated** concepts — things whose value is in the framework's assembly, like state messages (`register`/`emit`), later block actions. The `agentFacets` list stays **framework-core-only** ("how the framework talks to the agent at all", AGENTS.md), never an extension point.
...
- **app continuation** — the same substrate-owned queue as followUp; "run finished → substrate decides the next invocation" is a substrate (pilot) decision and composes with the fan-out direction, where a pi-side queue wouldn't.
- **The invariant underneath all of it:** _a human message never reaches the model without contemporaneous framework state._ Every moment a human utterance enters the model's reading order, the current canvas truth must enter beside it — which is what disqualifies both pi-native followUp and nextTurn for UIX.
