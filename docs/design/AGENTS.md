...
- **[agent-instance-state](./agent-instance-state.md)** _(resolved, explanation)._ Settled: an agent instance owns one session-branch viewpoint and lazy Pi runtime. The instance supervisor owns its lifecycle, while disposable guards protect every attachment, turn, and asynchronous use from teardown.
...
- **[host-workspace-runtime-boundaries](./host-workspace-runtime-boundaries.md)** _(exploring, explanation)._ A host owns process and platform integration and a workspace supervisor, while each workspace runtime owns exactly one workspace and its agent instances. A launcher above all runtimes projects workspaces and sessions, and an app is a host plus an explicit composition.
- **[pane-and-file-versioning](./pane-and-file-versioning.md)** _(resolved, explanation)._ Managed documents version behind DocumentStore; workspace files version on git through session worktrees and per-turn checkpoint refs, with checkpoint-on-leave restores, turn-state binding, and close-out reclaim.
- **[product-and-distribution](./product-and-distribution.md)** _(explanation)._ Product and distribution: UIX provides workspace runtime, client, and host infrastructure; core UIX serves engineers, Fruition packages an Electron host with an app composition, and a hosted product later runs one host instance per user in a VM.
...
- **[shared-live-object-lifetimes](./shared-live-object-lifetimes.md)** _(resolved, explanation)._ Supervisors exclusively own keyed live objects while independent guards let several holders prevent teardown without receiving lifecycle authority.
- **[uix-core-composition](./uix-core-composition.md)** _(exploring, explanation)._ Exploring UIX's Pi composition root, grouped feature facets, registry-to-installer boundary, reload reconciliation, typed communication, and base-tool composition.
