...
- **[agent-state-messages](./agent-state-messages.md)** _(exploring)_ — Exploring how cockpit/extension state reaches the agent and comes back typed: display-hidden custom messages with a system-prompt vocabulary now; a registerStateMessage/emitState substrate primitive, tools-as-output-contracts, CustomEntry event logs, and fan-out prepared roots as the axes.
- **[canvas-data-channel](./canvas-data-channel.md)** _(exploring)_ — Exploring the bidirectional canvas/document data channel: the anchored edit grammar, pane tools, writeback/user diffs, and the content-store seam (versioning is the sibling pane-and-file-versioning thread).
