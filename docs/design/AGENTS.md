...
- **[agent-session-routing](./agent-session-routing.md)** _(explanation)._ Agent and session routing: connections are URL-scoped to a workspace and session. Session switches re-target a connection in place. Agents are refcounted per workspace-session with a tunable TTL, and the host can author messages to them.
...
- **[product-and-distribution](./product-and-distribution.md)** _(explanation)._ Product and distribution: UIX is a runtime for agent apps. Three tiers consume one runtime: core UIX for engineers, Fruition as a batteries-included Electron composition, and a hosted product later. Local distribution is a native tray supervisor plus a SEA server binary.
