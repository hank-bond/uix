...
- **[agent-session-routing](./agent-session-routing.md)** _(resolved, explanation)._ A connection attachment owns a guard on one session-bound agent instance and dispatches with guarded authority. Turns and asynchronous operations hold independent guards, and the instance supervisor applies teardown policy only after every guard releases.
