...
This specification depends on [`managed-documents.md`](./managed-documents.md) for current content, immutable versions, and conditional replacement. [`feature-turn-state.md`](./feature-turn-state.md) defines branch persistence and restoration. [`viewpoint-web-namespaces.md`](./viewpoint-web-namespaces.md) defines browser delivery. [`connection-agent-attachments.md`](./connection-agent-attachments.md) defines request and event targeting. Canvas also follows the exported channel, Agent-tool, Agent-context, resource, and feature APIs.
...
The document store owns current content, revisions, immutable versions, atomic conditional replacement, and physical storage without interpreting HTML or anchors. Feature turn state controls when Canvas records and restores a selected version. Feature web routes dispatch browser requests to the correct Canvas instance through attachment-bound addresses. The complete-page URL establishes the feature root for directory-relative references without changing the response body. Workspace features provide reusable assets and application-specific handlers.
...
- Canvas document bytes **must** use viewpoint web routes instead of channel request payloads.
- The Canvas document GET route **must** declare an `html-document` response. Its route path **must** satisfy the complete-page path restriction in `viewpoint-web-namespaces.md`.
- The Canvas document GET route **must** select the Canvas key through typed query input. A nested Canvas key **must not** change the page's containing directory.
...
- The shim **must** resolve its directory-relative writeback URL from the page URL. Authored feature references use directory-relative addresses rather than origin-rooted paths.
- Authored HTML **must not** contain host domains, ports, attachment tokens, or an authored `<base href>` element.
- The substrate **must not** inject base elements or rewrite fragment links in the Canvas response body. Canvas owns any derived shim markup it adds before returning that body.
...
- The browser shim and embedded revision are derived content and **must** be omitted from serialized authored HTML. Canvas owns that omission.
...
8. The same Canvas document and directory-relative asset and route references work through Electron and web hosts without persisted physical URLs or Canvas-authored attachment logic. A nested Canvas key in the query does not change the resolution directory, and same-page fragment navigation retains the query without link rewriting.
...
- The iframe's physical URL within the complete-page addressing requirements.
