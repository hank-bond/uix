...
- **Pane doc tools** — `uix_canvas_read` / `uix_canvas_write` / `uix_canvas_edit` over a canvas key and current checkout, backed by the U1 core. (Every HTML pane document is a canvas, so the tools are canvas-named; the document/content abstraction stays underneath in the channel and store.) The model does not pass revisions; UIX tracks current internally and returns ids/results for observability.
