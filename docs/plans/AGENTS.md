...
- **[persistence-and-session-foundation](./persistence-and-session-foundation.md)** _(active)_ — Build spec for persistence on pi's session file: branch projection and feature-isolated restoration now run on startup and replacement-session activation; next, reload commits current active feature state before replacement and restores the reloaded branch into the replacement feature instances.
