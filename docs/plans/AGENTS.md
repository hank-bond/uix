...
- **[durable-transcript-identity](./durable-transcript-identity.md)** _(active)_ — Keyed-on-persist observation and row identity have landed (D0–D1); remaining work persists low-frequency block state as session-scoped uix.* custom entries (D2) and projects transcript/block state plus latest named turn-state-cell values through one branch pass shared with history and switching (D3).
