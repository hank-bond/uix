...
- **[durable-transcript-identity](./durable-transcript-identity.md)** _(active)_ — Keyed-on-persist identity, the one-pass transcript/turn-state-as-of-leaf projection, and feature-isolated restore scheduling have landed; remaining work persists and joins low-frequency block state (D2) and wires restoration into session activation (D3).
...
- **[persistence-and-session-foundation](./persistence-and-session-foundation.md)** _(active)_ — Build spec for persistence on pi's session file: session rehydration, versioned documents, keyed turn-state commits, branch projection, and restore scheduling have landed; next, startup, reload, and session activation invoke that scheduler before later preview/navigation.
