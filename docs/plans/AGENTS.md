...
- **[workspace-manifest-and-picker](./workspace-manifest-and-picker.md)** _(active)_ — Build manifest-driven workspaces: uix.workspace.json (name + ordered feature entry-file refs) replaces auto-discovery as what feeds the loader (M1), resolveWorkspace derives stateRoot/agentCwd from the opened manifest's directory (M2), and the App grows a start picker — recents from userData plus create-new that writes a manifest (M3).
