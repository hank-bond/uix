---
summary: "Build spec for persistence on pi's session file: session rehydration, versioned documents, keyed turn-state commits, branch projection, and restore scheduling have landed; next, startup, reload, and session activation invoke that scheduler before later preview/navigation."
status: active
---
...
2. Invoke the selected-branch projection and restore scheduler on startup, reload, and session activation; add preview only with the later navigation UI.
