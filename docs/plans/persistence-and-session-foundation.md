---
summary: "Build spec for persistence on pi's session file: file-backed session + history rehydration (C0), the in-process pi extension (C1), the first versioned store, contribution-keyed turn-state refs, and snapshot-derived canvas diffs have landed; next, named state cells contribute reason-free checkpoint plus restore together, with Canvas restore folding in anchor rehydration before later preview/navigation."
status: active
---
...
_Landed store:_ `DocumentStore` now has `getCurrent` / `setCurrent` / `snapshotCurrent` / `getVersion`; the local implementation stores current bytes and immutable JSON versions under `.uix/documents/`; `CanvasDocumentBuffer.snapshotCurrent()` (renamed from `DocumentChannel`/`DocumentBuffer` — it is the canvas feature's purpose-owned working copy over the store, not a transport or universal document abstraction) canonicalizes mutable latest and stores exact `AnchoredDocument` lines plus `nextAnchorIndex` as version meta; the overloaded `AnchoredDocument` constructor restores that state. The git-backed owned-pane store is still a later implementation behind the same seam, and its diff/delta compression is an optimization, not a semantic — the id-addressed `meta`-opaque seam keeps it deferrable.
...
**Build.** This is the **activation/navigation half of the symmetric state-cell contract** settled further in [session history and switching](./session-history-and-switching.md): a feature contributes one or more named cells, and each cell owns a reason-free `checkpoint()` plus `restore(latest | undefined)` together. The coordinator derives cell identity, persists only complete cell values that changed, projects each active cell's latest value from the selected branch, and routes it back to that cell; it remains store-blind while the cell resolves/applies its payload. Startup, reload, and session switching now provide the first restore triggers. Future read-only preview may remain a discrete callback because selecting a historical node should be able to show state transiently without committing a rollback. The same cell that checkpoints Canvas document refs restores content plus anchor metadata, so C4 lands inside Canvas restore rather than as a second session entry.

Activation **restore** lands first through startup, reload, and session switching, including Canvas content plus anchor metadata (this is where C4 now lands). Later read-only preview can select a past node and resolve its versions into the pane without changing the active branch. The later [pane-and-file-versioning](../design/pane-and-file-versioning.md) rollback action set (conversation-only / pane-only / both) drives `pi.fork` / `pi.navigateTree` (ExtensionCommandContext, available via C1); selective rollback remains future design over named state cells. = canvas-plan U5–U6. The opt-in user-file store (U7) is its own later plan.
...
2. Migrate turn state to named reason-free checkpoint/restore cells and run restore on startup/reload/session activation, with Canvas resolving snapshot refs back into content plus anchor meta; add preview only with the later navigation UI.
