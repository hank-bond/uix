...
_Update 2026-06-21:_ the submit/agent-end appending moved out of the canvas facet into the substrate-level `StateRegistry` + `createStateCoordinator` (`src/main/state/`); the canvas now provides a `StateContribution` (`src/main/canvas/contributions/state.ts`) registered by the state substrate. Next: make the contribution contract **symmetric** with preview/restore callbacks beside prepare, which is also what generalizes turn-state to contribution-keyed opaque state — see C5 and [pane-and-file-versioning](../design/pane-and-file-versioning.md) Log 2026-06-21. In parallel, the canvas path is being reframed as a default feature: it will contribute `canvas` state, a canvas document kind, exact source ids such as `canvas.pane.writeback` / `canvas.agent.anchor_edit`, and listeners that decide refresh vs agent-visible diffing from those source ids.
...
1. Change `StateContribution` from flat `panes` to contribution-keyed opaque state: `uix.turn-state` should write `{ cwd, state: { canvas: { "doc://canvas/main": snapshotId } } }`.
