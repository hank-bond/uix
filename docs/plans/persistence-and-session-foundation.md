...
**Build.** This is the **navigation half of the symmetric contribution contract**: each contribution provides restore/preview callbacks beside its prepare callbacks, and the coordinator routes a node's contribution state back to its owning contributions (store-blind — only the contribution resolves/applies its payload). Adding these callbacks is deferred while UIX stays single-branch, but the persisted state shape is chosen now so navigation can be added without changing history. Preview and restore may remain discrete callbacks because selecting a historical node should be able to show state transiently without committing a rollback. Adding this hook is also what generalizes prepared state to contribution-keyed/opaque (see [pane-and-file-versioning](../design/pane-and-file-versioning.md) Log 2026-06-21), so C5 lands the contract generalization and restore together rather than generalizing speculatively first.

Read-only **preview** first (select a past node → the canvas contribution's preview hook resolves its versions + anchor state into the pane so the human sees it as it was; this is where C4 lands). Then the [pane-and-file-versioning](../design/pane-and-file-versioning.md) rollback action set (conversation-only / pane-only / both), driving `pi.fork` / `pi.navigateTree` (ExtensionCommandContext, available via C1) — and because state is per-contribution, selective rollback (canvas-only vs files-only) is just invoking a subset of restore hooks. = canvas-plan U5–U6. The opt-in user-file store (U7) is its own later plan.
...
2. Add the navigation half of the turn-state contract: preview/restore callbacks beside prepare, with canvas resolving snapshot refs back into content plus anchor meta.
