---
summary: "Build spec for persistence on pi's session file: file-backed session + history rehydration (C0), the in-process pi extension (C1), and the first versioned-store + central state-coordinator slices (C2/C3) are landing; the state-message substrate is transitional, and the next step is the symmetric contribution contract — a restore hook beside prepare — which generalizes turn-state to contribution-keyed/opaque slices and folds anchor rehydration (C4) into canvas restore (C5)."
status: active
---
...
## C2 — Versioned content store · **first JSON-object slice landed 2026-06-17**
...
**Build.** This is the **restore half of the symmetric contribution contract**: each contribution provides a restore/preview hook beside its prepare hook, and the coordinator routes a node's slices back to their owning contributions (store-blind — only the contribution resolves its refs). Adding this hook is also what generalizes the prepared slice to contribution-keyed/opaque (see [pane-and-file-versioning](../design/pane-and-file-versioning.md) Log 2026-06-21), so C5 lands the contract generalization and restore together rather than generalizing speculatively first.

Read-only **preview** first (select a past node → the canvas contribution's restore hook resolves its versions + anchor state into the pane so the human sees it as it was; this is where C4 lands). Then the [pane-and-file-versioning](../design/pane-and-file-versioning.md) rollback action set (conversation-only / pane-only / both), driving `pi.fork` / `pi.navigateTree` (ExtensionCommandContext, available via C1) — and because slices are per-contribution, selective rollback (canvas-only vs files-only) is just invoking a subset of restore hooks. = canvas-plan U5–U6. The opt-in user-file store (U7) is its own later plan.
