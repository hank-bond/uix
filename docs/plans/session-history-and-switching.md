...
Session switching, reload, and future branch rewind share one state transition lifecycle. New Session is a substrate-provided workspace action, so its invokable action guard reads the workspace's current agent state regardless of whether Chat is installed. While busy, the action skips the backend request; if activity races the renderer guard, main rejects rather than aborting the run. Transient notice presentation remains later polish. Reload and switching adopt their own reviewed busy policies when their action and selector paths land rather than inheriting an abort policy speculatively.
