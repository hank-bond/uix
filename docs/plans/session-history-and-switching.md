...
Register substrate-owned New Session with `mod+n` beside Reload so it exists without Chat. Keep the action invokable but guard on current workspace agent state; if busy it does not call the backend. Rich transient-notice presentation follows later rather than blocking this slice. Verify a new session never clones the previous session's turn-state refs.
