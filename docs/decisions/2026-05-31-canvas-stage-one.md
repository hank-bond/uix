...
- **Own-origin protocol, not `srcdoc`.** HTML served from a stable `uix-canvas://` URL with key segments reversed into the host. Each key then has its own origin, and the path stays free for future fragments. Loaded via `src`: `srcdoc` would inherit the host origin and let agent HTML reach `window.uix`/host DOM. `allow-scripts allow-same-origin` is safe precisely because "same-origin" means the canvas's own origin, which holds nothing privileged.
