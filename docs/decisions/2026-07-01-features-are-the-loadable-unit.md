...
Two parallel systems grew side by side. The extension loader (discovery, jiti entry loading, per-entry bags, error isolation, reload) handed entries a stub `ExtensionAPI`. The feature contribution system (`FeatureDefinition`, facet registries, `registerFeatureContributions`) was fed by a hardcoded bundled inventory. This decision converges them into one system under one word.
...
- _Keeping both systems_ (extensions for external, features for bundled): two names and two registration paths. The API boundary (`ExtensionAPI`) would have to grow into a worse duplicate of `FeatureContributions`.
- _Growing `ExtensionAPI` into the real API_ (imperative `register*` methods): registration order becomes author-emergent. Lifetime enrollment becomes side-effectful per call. Every facet needs a method mirror of its contribution type. The declarative definition already exists and is what canvas ships on.
