---
summary: "Host reload tears down and re-activates extensions through an extensionsBag child lifetime without restarting Electron, delegating to Pi's session.reload()."
kind: explanation
...
- **Host-level trigger.** Main exposes `uix:reload` through the preload bridge as `window.channels.reload()`. It reloads the UIX tree and, if a Pi session already exists, delegates to Pi's own `session.reload()`. It deliberately does not create a session just to service reload. No throwaway renderer button. The hook is ready for a future command palette / `/reload` chat command.
...
> Note (2026-08-02): the preload bridge exposes the transport as `window.channels` (with a `reload()` method), not `window.uix`. The `uix:reload` channel name is unchanged. The Host-level trigger bullet was corrected in place.
