...
- **Host-level trigger.** Main exposes `uix:reload` through the preload bridge as `window.uix.reload()`. It reloads the UIX tree and, if a pi session already exists, delegates to pi's own `session.reload()` — it deliberately does not create a session just to service reload. No throwaway renderer button; the hook is ready for a future command palette / `/reload` chat command.
