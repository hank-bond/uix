...
This rule applies at the owner boundary rather than the physical-file boundary alone. One managed document write is atomic, and one settings mutation is atomic. The workspace configuration read by `/reload` is atomic too, even though `uix.workspace.json` contains several domains. Collecting all useful validation errors is preferred to failing at the first one. Diagnostics never authorize partial application.
...
**Rejected:** best-effort field application, ignoring invalid entries, and retaining old values for only the invalid portions of a new candidate. Also rejected: aborting every sibling because one feature's runtime activation failed, and claiming rollback for side effects outside substrate ownership.
