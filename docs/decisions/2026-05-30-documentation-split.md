...
- **`src/docs/`**, user-facing: what the code is and how to use it. That covers writing an extension, contributing a pane, defining a channel, and integrating with the agent session. Ships with the substrate. If a doc here is wrong, either the doc or the code is broken. Update them together, in the same commit.
