...
| Language | TypeScript everywhere | None | One language across main, preload, renderer, extensions. |
...
| Virtualization | @tanstack/virtual | None | Non-optional once trees/chats/reports have many items. |
| Workers | Web Workers + Comlink | None | JSONL parsing/indexing without blocking UI. |
| Storage | better-sqlite3 (derived index) + files (truth) | None | Cross-session queries are slow on JSONL. SQLite is a rebuildable mirror. |
| File watching | @parcel/watcher | chokidar | Faster at scale. |
| Tests | Vitest | None | None |
...
| Not picking | router, async query lib, CSS-in-JS, large component library | None | Resist accidental complexity. |
