...
**The test.** When tempted to hardcode a UI feature, ask whether it should instead be a primitive that something registers or composes. The host's default chrome is the bare-bones baseline — and it is itself built on the same public primitives (first-party, no privileged path), so anything the defaults can do, an extension can too. This is the visual-UI restatement of [uix-is-a-framework-not-an-app](./2026-05-30-uix-is-a-framework-not-an-app.md): define small primitives, not big components, and compose.
