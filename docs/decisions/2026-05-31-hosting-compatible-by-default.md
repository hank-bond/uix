---
summary: "Designs stay survivable under a future hosted/VM architecture: the filesystem is one local impl behind content-store and change-feed interfaces — address by id, host is sole writer, content-hash echo suppression, field-level merge."
kind: explanation
...
- **The host is the sole writer**, and every writer announces itself over a host-owned channel (writeback, the agent via the pi event stream, the form shim, the Monaco source pane). The file is a persistence format, not a coordination point. Identical invariant hosted (server is sole writer).
