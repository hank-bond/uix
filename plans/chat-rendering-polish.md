...
- Make each tool row include its point-in-time execution cwd. Make each relevant filesystem row include a `ToolFileLocation`: a lexical absolute path plus a cwd-relative-or-absolute display path.
...
- After the complete model, cwd, and thinking-control shape is visible, reassess `src/main/agent/driver.ts` for discrete ownership units. Extract only boundaries earned by the implemented behavior.
