---
summary: "Archived host-split build: Electron and web hosts now compose one workspace runtime and shared browser client. The attempted synthetic conformance gate was canceled in favor of future real-host behavioral testing."
read_when: "Read for the implementation history, review units, or lessons behind the Electron/server host split."
status: archived
---
...
_Status: canceled after attempt 1._

This unit proposed one semantic suite over in-memory, Electron, and web-host adapters. The attempted implementation built a test-owned fake runtime and normalized each physical host behind another test adapter.

**Lesson learned:** The scenario list was useful, but the mechanism tested the fake runtime and adapters more directly than either real host. It duplicated runtime behavior and created a second contract to maintain. It could give false confidence while real preload, process, renderer, protocol, and packaging paths remained untested. The existing focused unit tests are the right level for transport and lifetime edge cases. Host-wide confidence should come from functional behavioral tests that launch the actual Electron and server hosts.

The attempted conformance fixture and adapters were removed before review. A complete behavioral suite is deferred to its own future plan, after the accepted specifications have complete requirement coverage. H8 no longer blocks completion of the host split.
...
- A complete cross-host functional behavioral suite. That work follows complete specification coverage under its own plan.

## Completion gate

The split completes when Electron and the web host are discrete hosts over one workspace runtime and shared browser client. A browser can open a workspace-only URL, create and revisit sessions, and run a basic Chat and Canvas flow across the control/content split. It can reconnect and rehydrate after a drop, and reload edited feature source. Electron preserves current behavior from its own composition root. Host fields do not enter feature contracts, and the loopback and non-loopback deployment profiles satisfy the same web-host contracts. Focused unit and integration tests remain with their owning subsystems. Full host behavioral testing is intentionally deferred to a separate plan.
