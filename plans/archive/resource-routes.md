...
Resource contributions should describe **what location shape they serve**, not hand-build protocol URLs. The resource facet should derive the resource type address (`canvas-doc` today) and generate renderer URLs. It should parse incoming requests and hide Electron custom-protocol vs future web-server hosting from feature code.
...
The substrate provides the inverse URL builder. The helper name may change once it moves behind a Workspace resource client. The operation is deterministic conversion from resource type + route params into a transport URL:
...
- **Origin policy is explicit.** Canvas requires iframe origin isolation from the workspace/host. The route contract therefore says whether a resource belongs to the shared workspace origin or to the feature's isolated origin. It does not hide that decision in a feature-owned host codec.
...
The registry wraps `protocol.handle` callbacks and parses the request URL against the registered route. It validates query payloads and returns a 404/400-style response for non-matches or invalid locations.
