...
The value-first slice of the [canvas-data-channel](../../docs/design/canvas-data-channel.md) thread: prove a working bidirectional canvas end-to-end on Pi's `customTools` **before** the lower-level integration surgery. That refactor (U3) then lands against a working proof rather than ahead of it. Assumes the landed constraints in [hosting-compatible-by-default](../../docs/decisions/2026-05-31-hosting-compatible-by-default.md), [canvas-stage-one](../../docs/decisions/2026-05-31-canvas-stage-one.md), and [no-agent-ui-manipulation](../../docs/decisions/2026-05-30-no-agent-ui-manipulation.md).

**Scope of this plan:** P0–U2 below. **Out of scope (later units, separate plans):** U3 lower-level Pi refactor + Half-B context dedup. U4 filesystem-tool parity. U5–U7 versioning + rollback (→ [pane-and-file-versioning](../../docs/design/pane-and-file-versioning.md)).
...
- **Content-store interface**, `getCurrent(docId)` / `setCurrent(docId, content)` / `snapshotCurrent(...)` / `getVersion(...)`, with a **trivial current-plus-version implementation**. This is the reserved seam: the git-backed versioned store (U5, [pane-and-file-versioning](../../docs/design/pane-and-file-versioning.md)) slots in here later **without touching the channel**.
