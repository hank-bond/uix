...
Migrate one coherent multi-file source ownership boundary together with the generator. Add its production-file headers and `AGENTS.md`, and move caller-facing contracts into JSDoc. Preserve cross-file coordination in directory guidance or a local leaf. Remove duplicate descriptive prose only after preserving non-obvious knowledge.
...
_Landed: the `src/main` and `src/renderer` boundaries migrated together with the generator (`3302d24 refactor: align source docs with ownership boundaries`). They produced the current source `AGENTS.md` routing files._

## Ongoing migration: split every src/docs/ page · **landed 2026-08-04**

The goal is to migrate: split every page in `src/docs/` into exactly one of two destinations. Continue migrating remaining ownership boundaries during ordinary repository work. Where the how-tos finally rest is deferred until the split completes.
...
When the last page moves or is deleted, remove the transitional "shipped reference" route and its "Update when" note from `docs/AGENTS.md` and `docs/style-guide.md`. Also update `docs/architecture/current-state.md`'s reference to the shipped reference.
