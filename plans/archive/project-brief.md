...
This frame is the single most important design constraint. UIX is not adding agent capabilities. It is adding **user capabilities**: displays the human sees, controls the human's hands operate, and memory aids. Those memory aids mean the human doesn't have to hold everything in their head.
...
1. **TUI agents force linear, scroll-heavy interaction.** When the agent asks 5 questions at the top of a long response, you have to scroll up to read them. Then you scroll back down to answer. The interaction model is anchored to "type at the bottom" and it's wrong for anything richer than chat.
...
3. **Side-quest workflows are clunky.** When something goes wrong mid-conversation, the user wants to branch off, reflect, and update skills. Then they return to the bad turn, rerun with the updated skill, and continue. Pi's primitives support this. The UX around it doesn't really exist anywhere.

4. **No queue for "fix this later."** When the user notices a problem but doesn't want to stop now, there's no good way to bookmark it. "Come back and reflect on this" cannot carry across sessions.
...
Reports are **separate, structured artifacts the agent generates by writing files**. They are not "rich chat messages." They are documents the user opens in their own pane/tab. They are how the user reviews and interacts with substantial agent work.
...
- When the user clicks submit, the **app** posts a message into the conversation pane (on the user's behalf). The message reads "Submitted answers in `report-42.md`: q1, q3, q5. Skipped q2, q4." The agent reads the file for details.
...
**Phase 2 option**: if Pi running in main ever becomes a stability concern, Electron's `utilityProcess` can isolate it into a managed Node subprocess. It still uses the SDK. Don't do this day one.
...
**UIX is an Electron + React framework around Pi. It turns Pi's already-rich session tree and file-editing primitives into a 2D workspace for reviewing, navigating, and steering coding agents. Structured reports are the central artifact, and tree-based side-quest workflows are the central interaction pattern.**
