...
- [Atomic candidates and feature activation](../docs/decisions/2026-07-13-atomic-candidates-and-feature-activation.md): workspace settings reload commits one validated candidate. A failed feature leaves none of its settings or facet registrations behind and does not abort siblings.
...
- Existing materialized bindings beat contributed defaults. The registry retains stale ids harmlessly.
...
- **Actions are frontend effectors.** Every action handler registers and executes in the renderer. A human-triggered backend operation is simply an action callback composed with a typed channel request. Its backend handler and later frontend event subscribers remain ordinary channel concepts. A separate backend action-handler facet would duplicate the typed request/event SDK and create a second invocation system.
- **Colocated authoring, split projections.** `defaultBinding` stays on the authored action leaf. Resolution deterministically emits a private `ResolvedActionContribution`, a public JSON-safe catalog entry, and a default-binding template entry with the same derived id and lifetime. This keeps the handler ignorant of bindings while preventing the drift and extra authoring ceremony of two matching action/default trees.
- **Frontend-declaration reconciliation.** After the renderer registers actions, and again only when the default-template projection changes: it batches `{ actionId: defaultBinding }` declarations to main. Main overlays its current durable map, validates/persists when changed, and returns the complete confirmed snapshot. Defaults necessarily originate where frontend actions are declared. This handshake preserves main as the sole durable owner and remains idempotent for late-mounted actions or reload.
...
- **Derived conflicts fail closed.** The renderer resolves `mod` for its own platform and groups active actions by normalized shortcut. It derives every other claimant into `conflictsWith`. It dispatches none of them by keyboard while preserving direct id invocation. Conflicts depend on the current client and active composition, so persisting or resolving them by load order would create stale or surprising behavior.
- **One portable chord in v1.** Canonical strings use `+` within a chord and `mod` for Command on macOS, Control elsewhere. They use the established `ctrl`/`alt`/`shift` plus letter, digit, named navigation/editing, arrow, and function-key vocabulary. This follows VS Code/Mousetrap/CodeMirror/Pi conventions with a small internal parser and browser adapter. It avoids a dependency that would not cover UIX persistence, catalog, conflict, or context semantics.
- **Sequences are a compatible later extension.** The parser reserves and rejects spaces in v1, while the internal chord representation permits an empty modifier set even though v1 policy rejects it. Later `g g`/bare-key command modes can therefore add pending state, timeout, Escape/context eligibility, and prefix rules without migrating existing bindings or redesigning parsing.
...
The pure shortcut grammar now validates and canonicalizes portable one-chord gestures. It resolves `mod` for a client platform and keeps produced characters out of persistence (`shift+1`, not `!`). The composition root also registers `settings.keybindings` as the closed canonical-action-id record `Shortcut | null` with an explicit `{}` default. Invalid external candidates retain the prior generation. Bound main request handlers serve the two-request `uix` channel. Reconciliation with `{}` doubles as read. Complete candidate replacement validates before one persistence write. Changed maps publish one confirmed snapshot. Action resolution now splits private `ResolvedActionContribution`, public catalog-entry, and canonical default-binding values. `ActionRegistry` maintains and publishes an aggregate default-template snapshot independently from enabled/running catalog churn. Renderer `KeybindingSync` subscribes to confirmed snapshots before its initial reconciliation. It batches same-turn template changes and reconciles again after substrate reload. It rejects stale responses after newer events. It keeps confirmed bindings explicitly unhydrated until main responds. Hydrated bindings join active catalog entries on the renderer platform. Every conflict claimant is marked. Inactive persisted ids appear separately in the unresolved projection without losing their durable values.

### Remaining implementation

Expose renderer whole-scope binding replacement through the narrow workspace action API. Future keyboard dispatch remains gated on the confirmed snapshot.
...
- One authored action contribution deterministically produces a private resolved contribution, public catalog entry, and default-binding template with identical ids and lifetimes. Enabled/running-only updates trigger no default change.
...
Ambient surfaces use the existing module pipeline, typed channel binding, feature settings provider, scoped/adopted styles, error isolation, and reload lifetime. They receive no panel/header or resizable-layout allocation. Mount them under a real `[data-uix-surface]` root so descendant native dialogs retain scoped styles in Chromium's top layer.
