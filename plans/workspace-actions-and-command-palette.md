...
- **One portable chord in v1.** Canonical strings use `+` within a chord, `mod` for Command on macOS and Control elsewhere, and the established `ctrl`/`alt`/`shift` plus letter, digit, named navigation/editing, arrow, and function-key vocabulary; the parser accepts and normalizes modifier order, rejects duplicates, and requires at least one modifier. This follows VS Code/Mousetrap/CodeMirror/Pi conventions with a small internal parser and browser adapter rather than adding a dependency that would not cover UIX persistence, catalog, conflict, or context semantics.
