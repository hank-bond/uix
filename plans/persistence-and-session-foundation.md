...
Activation **restore** now runs on startup, replacement-session rebind, and feature reload, including Canvas content plus anchor metadata (this is where C4 lands). Reload uses a transient turn-state registry snapshot so an obsolete deferred bootstrap request cannot restore replacement instances or mark them ready; reload invocations are serialized, and the renderer receives `surfaces_changed` only after replacement restoration settles. Later read-only preview can select a past node and resolve its versions into the pane without changing the active branch. The later [pane-and-file-versioning](../docs/design/pane-and-file-versioning.md) rollback action set (conversation-only / pane-only / both) drives `pi.fork` / `pi.navigateTree` (ExtensionCommandContext, available via C1); selective rollback remains future design over named state cells. = canvas-plan U5–U6. The opt-in user-file store (U7) is its own later plan.
