---
summary: "Build spec for persistence on Pi's session file. Branch projection and feature-isolated restoration run on startup, replacement-session activation, and serialized feature reloads, with current state committed before replacement."
---

# Spec: persistence + session foundation (C0–C5)

Persistence is the phase that ties the conversation tree, canvas versions, and anchor state together. Frame and rationale: [session-file-as-state-substrate](../docs/decisions/2026-06-06-session-file-as-state-substrate.md) (Pi's session file is the substrate). Versioning mechanics behind the document-store seam: [pane-and-file-versioning](../docs/design/pane-and-file-versioning.md). The conversation-render work this unblocks: [conversation-render-primitives](../docs/design/conversation-render-primitives.md).

> **H4 update:** Each `AgentInstance` now owns its Agent feature factories, facet registries, Canvas buffer, and turn-state coordinator. Attachments on one session share that viewpoint. Different sessions do not share mutable Canvas state.

**Do first, then context-clear:** **C0 + C1** are the foundation to lay before resuming conversation-render improvements. C0 changes the shape the renderer consumes (replayed complete entries, not just live deltas). C1 puts that work on the final substrate, the Pi `ExtensionAPI` (`sendMessage`/`registerMessageRenderer`/message-transforms), so host-authored blocks can be real session entries from day one. **C2–C5 are deferred** but specified below so nothing is lost.

## Spike answers (resolved 2026-06-06)

- **Write handle.** A Pi `ExtensionFactory = (pi: ExtensionAPI) => void` gives the live `ExtensionAPI`: `registerTool`, `appendEntry`, `sendMessage`, `registerMessageRenderer`, `on(...)` hooks. Load in-process with no file discovery: `new DefaultResourceLoader({ cwd, agentDir, extensionFactories: [uixCore] })` → `await loader.reload()` → `createAgentSession({ resourceLoader })`. This is Pi's extension system, **not** UIX's frontend one (`src/main/extensions/`, `@uix/api`).
- **Leaf / parent.** `appendEntry(customType, data)` takes no `parentId`: Pi auto-attaches to the current leaf. `getLeafId()` / `getLeafEntry()` (on `ReadonlySessionManager`, hence on tool `ctx.sessionManager`) read the current leaf for correlation. Hook choice sets which node a pointer attaches to: `input` (fires before processing → parents to the previous leaf) vs `turn_start`/`turn_end` (within the new turn). Settle in C3.

---

## C0: File-backed session + history rehydration · _foundation, do first_ · **landed**

**Goal.** The conversation survives restart.

**Build.**

- **Two roots, not one.** A stable **state root** holds the `.uix` tree: the document store and the Pi session file via an explicitly pinned `sessionDir`. The **agent cwd** is what Pi's coding tools operate against. Equal today, but kept distinct so a later worktree shift can move the agent cwd without relocating canvases or the session file. See [open-questions](../docs/architecture/open-questions.md) "Reactive agent cwd". The local document store is rooted at the UIX state root. Keep that distinct from the agent cwd used by Pi tools.
- **Resume, not just file-backed.** Swap `SessionManager.inMemory()` (`src/main/agent/driver.ts`) for resume-or-create: continue the most recent session under the state root, creating one only when none exists. File-backing alone would start empty every launch. "Survives restart" means resume.
- **Progressive, non-blocking load.** Reading prior history needs only the session file, not auth/model. The cheap read is eager (off the boot path). The auth-bearing live agent stays lazy until first prompt. App paint never blocks on session construction. This is also the seam the reactive cwd swaps against (rebuild the lazy tier, leave the eager read and canvas state alone).
- **Startup rehydration.** On startup, read the persisted branch (`getBranch`/`buildSessionContext`) and seed the renderer transcript. _Landed follow-up:_ main now normalizes both complete historical entries (messages plus tool calls/results and displayed custom messages) and live streaming events into one `TranscriptItem` shape. Live partials remain in-flight only and are discarded when the final item arrives.

**Boundary.** No versioning, no custom entries, no extension promotion. Pure Pi read + file-backed session. Lands before any render improvement, or the renderers get retrofit when history rehydration arrives.

## C1: Promote UIX-core agent facets to an in-process Pi extension · _foundation, do first · = canvas-plan U3_ · **landed 2026-06-07**

**Goal.** Hold Pi's `ExtensionAPI`: write access to the session tree, hooks, and the message-transform seam.

**Build.**

- Author a **UIX-core Pi `ExtensionFactory`**. Move the canvas read/write/edit tools off `createAgentSession({ customTools })` into the factory via `pi.registerTool(...)`. They can technically stay `customTools`, but moving them keeps the API handle and tools together. _Landed: `createUixCoreExtension` in `src/main/agent/facets.ts` composes an ordered list of `AgentFacet` functions, each handed the live `pi`. The canvas facet (`createCanvasAgentFacet`) registers its read/write/edit tools via `pi.registerTool`._
- Migrate `contextForTurn` (today a manual prepend in `driver.ts`) to `pi.on("input", ...)` returning `{ action: "transform", text }`, the native submit-boundary hook. The human-writeback diff prepend becomes a transform. The human's original message entry is untouched. _Landed: the driver now sends the human's text verbatim and the canvas facet's `input` hook prepends context for the model._ _**Corrected 2026-06-11:** the "entry is untouched" claim was false. Pi persists the transformed text as the user entry (dist source: `prompt()` builds the user message from the transform's output). The prepended context therefore sat inside the human's own message. The `input` transform is gone. Turn context currently rides display-hidden custom messages flushed at `before_agent_start`. The target C3 path moves them into pre-user submit prep: see [agent-state-messages](../docs/design/agent-state-messages.md) and the C3 notes below._
- Wire `DefaultResourceLoader({ extensionFactories: [uixCore] })` into session construction. Preserve reload semantics via `resourceLoader.reload()` + `session.reload()`. _Landed in `driver.ts` `openSession()`. **Deviation:** `session.reload()` already reloads the resource loader internally (`agent-session.js`), so the host `reload()` was left calling `session.reload()` alone: no separate `resourceLoader.reload()` needed._
- Bridge the `pi` `ExtensionAPI` handle out to the host code that will need `appendEntry`/`sendMessage` (the store/driver), so C3+ can write entries. _**Deferred until the first real consumer.** Exposing the handle now would be a consumer-less accessor (dead code against the repo's "build the contract, not speculation" ethos). The factory captures it the moment the first `uix.*` entry write needs it: [durable-transcript-identity](./durable-transcript-identity.md) D2's block state or C3's `uix.turn-state`, whichever lands first._

**Boundary.** Substrate swap + `contextForTurn` migration only. No new persisted state yet (that is C3+). No user-visible change. Keep `session.subscribe` as the renderer's event source: the `on(...)` hooks are an addition, not a replacement.

> **Scope note.** This landed as the composition-root structure, not just a narrow swap: `createUixCoreExtension` runs an ordered list of per-subsection `AgentFacet` functions, each handed the live `pi`. Order is load-bearing because Pi has no priority field: rationale in decision [uix-core-composition-root](../docs/decisions/2026-06-07-uix-core-composition-root.md) and design thread [uix-core-composition](../docs/design/uix-core-composition.md). Read those before adding a _second_ facet.

## C2: Versioned content store · **first JSON-object store landed 2026-06-17**

**Goal.** Each Agent viewpoint has a mutable Canvas working copy over shared immutable versions. A version restores content and anchor state together.

**Build.** Evolve `DocumentStore` (`src/main/document-store.ts`) into two concepts behind one seam. `getCurrent` / `setCurrent` keep the latest mutable document bytes current on every iframe writeback or agent tool write. `createSnapshot(content, meta)` returns a version id, and `getVersion(id)` restores an immutable version. A version is **commit-like, not a bare blob**: `{ contentRef, meta: { anchorMap, allocIndex } }`, content blobs stay content-addressed and deduped underneath. The version id names the commit object, **not** the content hash, because two identical contents can have different anchor states. Anchor state rides the version because it is a function of the document's edit history up to that commit. Restoring a version is therefore atomic and necessarily consistent (see revised C4). Simplest durable impl behind the seam (objects under `.uix`). The git-plumbing owned-pane store ([pane-and-file-versioning](../docs/design/pane-and-file-versioning.md)) slots in later behind the same seam, conveniently the same commit-object shape. Diff/delta compression is explicitly an optimization deferred to that store (packfiles), never a semantic. If git mechanics require object creation on every writeback, those versions are ephemeral until a session `CustomEntry` points at them. Only referenced snapshots are branch state.

_Landed store:_ `DocumentStore` has `getCurrent` / `setCurrent` for Workspace uses plus `createSnapshot(content, meta)` / `getVersion` for immutable JSON versions under `.uix/documents/`. Each Agent factory creates one `CanvasDocumentBuffer`. The buffer keeps mutable HTML and anchors local, writes exact content and `AnchoredDocument` state into immutable versions, and restores that state from turn-state refs. The git-backed store remains a later implementation behind the same id-addressed seam.

**Boundary.** Store-only. Session linkage starts in C3.

## C3: Submit-boundary entries: turn-state pointers and human canvas diff

**Goal.** Each turn records which document snapshots **and which agent cwd** were live. The human's pending canvas edits reach the agent as a durable non-user message. Both ride the tree as entries, prepared at one boundary and ordered before the user message.

**Build.** UIX submit preparation runs after the latest Canvas writebacks update the selected Agent buffer. It appends the following entries before calling `session.prompt(text)`:

- **`uix.turn-state` pointer:** the turn-state coordinator appends `pi.appendEntry("uix.turn-state", { cwd, state: { [featureId]: <opaque> } })`. The canvas feature's state is `{ "doc://canvas/main": snapshotId }`, but the coordinator never interprets it. See [pane-and-file-versioning](../docs/design/pane-and-file-versioning.md) Log 2026-06-21 for the contribution-keyed/opaque-state model that supersedes the old flat `panes` shape. On resume/navigation, the nearest such entry up the branch says which snapshots were current and which cwd the agent was at. Reopen at that cwd (fall back to the home root + notice if the path is gone). Restore granularity is **run boundaries**: matching Pi CLI's model, which is why pointers are per-boundary even though the latest file changes continuously. Resolves [pane-and-file-versioning](../docs/design/pane-and-file-versioning.md) open-Q #3 and holds the per-turn cwd (substrate-owned, since the store is path-unaware) that [project-root-vs-agent-cwd](../docs/decisions/2026-06-06-project-root-vs-agent-cwd.md) depends on.
- **`uix.state` / `<canvas.canvas-diff>` as a `CustomMessageEntry`**: derive the anchored human-diff from the nearest upstream `uix.turn-state` snapshot to the new submit snapshot. `content` is the `<uix-state>` envelope with `<canvas.canvas-diff>` and any other state sections. `details` is the structured sidecar for rich/debug rendering. `display` controls the human-facing strip. It is appended **before** the user message lands. Tree navigation to the gap before a user message still has the hidden state needed to hydrate that point. The stored user entry stays exactly what the human typed. _**Current implementation note:** the derivation has landed in the agent-context substrate (`src/main/agent-context/`). Materializers run after turn-state prep and read their own feature's latest/prior `uix.turn-state` refs. Canvas computes anchored hunks from persisted snapshot meta. The delivery seam is still transitional: the hidden message is flushed from `before_agent_start`, which Pi orders after the user message. The desired submit-prep path still appends the custom message before `session.prompt(text)`._ At `agent_end`, create snapshots of loaded canvas working documents and commit changed cells in `uix.turn-state`. The next user-submit diff then uses that post-agent snapshot as the nearest upstream baseline, because the agent already observed those changes through tool results. Canvas does not separately report which document is visible. The store keys document updates by document resource id. Future surface interactions will carry surface-instance identity. The hidden `uix.state` message still flushes through the transitional `before_agent_start` assembler, so ordering that custom message before the user entry remains open.

_Update 2026-06-21:_ the submit/agent-end appending moved out of the canvas facet into the substrate-level turn-state registry/coordinator (`src/main/turn-state/`). _Update 2026-06-30:_ the consuming `consumeChanges()` path is gone. Canvas turn state stores only branded `doc://canvas/...` resource ids mapped to snapshot ids. Agent context derives `<canvas.canvas-diff>` from the current/prior snapshot refs at materialization time. _Update 2026-07-18:_ the old singleton contribution became keyed schema/snapshot/restore cells. Canvas now contributes `canvas.documents`. The coordinator validates snapshots and independently suppresses unchanged complete cell values, and its restore callback resolves content plus anchor metadata or resets defaults. Shared branch projection and feature-isolated restore scheduling now cover startup, replacement-session activation, and serialized feature reloads. Reload commits the active feature state when its prior restoration settled. It activates replacements and reloads initialized Pi resources. It restores the selected branch into the replacement activated feature instances, then publishes the changed surface composition. In parallel, the canvas path is being reframed as a default feature. It will contribute `canvas` state, a canvas document kind, and exact source ids such as `canvas.pane.writeback` / `canvas.agent.anchor_edit`. It will also contribute listeners that decide refresh vs agent-visible diffing from those source ids.

Both entry types rehydrate through the one branch-walk projection ([durable-transcript-identity](./durable-transcript-identity.md) D3). Each entry's projector lives beside the facet that writes it.

**Boundary.** Record + read. Restore UI is C5. The agent `changeCwd` capability and standardized worktree creation are their own work, gated on an app needing the move.

## C4: Anchor rehydration from version meta · **landed 2026-07-19**

**Goal.** Resumed/navigated sessions keep anchor identity, so historical anchors in the transcript still resolve and the edit match-guard works without forcing a re-read.

**Build.** Anchor state (anchor↔line map + allocation index) is **stored in the version's commit meta (C2), not as a session entry**. It is a function of the document's edit history up to that commit. Checkout therefore restores content and anchors as one atomic, necessarily-consistent unit. The C3 turn-state pointer alone stitches turn → version → `{content, anchors}`. C4 reduces to the rehydration wiring: on resume/branch navigation, seed the reconciler from the checked-out version's meta. This supersedes both the earlier `uix.anchor-state` entry idea and the loose sidecar cache. **This is no longer a separate unit. It _is_ the canvas contribution's restore hook** (C5). Resolving `getVersion` hands back content + anchor meta together. Anchor rehydration is therefore just what restore does for canvas.

**Droppability is the safety story, not the mechanism.** If meta is missing/stale, regenerate from content, consistent but **renumbered**, which dangles every anchor quoted in historical tool results. The edit match-guard then rejects rather than corrupts and the agent re-reads. That re-read dumps the document (or requested section) back into context, the cost that makes anchor continuity worth engineering and the guard a last resort.

**Boundary.** Continuity only.

## C5: Tree preview and restore · **activation restore landed. Preview remains**

**Goal.** Move around the conversation tree. Canvas content and anchor state follow.

**Build.** This is the **activation/navigation half of the symmetric state-cell contract** settled further in [session history and switching](./session-history-and-switching.md). A feature contributes one or more named cells. Each cell owns a reason-free `createSnapshot()` plus `restore(latest | undefined)` together. The coordinator derives cell identity and persists only complete cell values that changed. It derives each active cell's latest value in the selected-branch projection and routes it back to that cell. It remains store-blind while the cell resolves/applies its payload. Startup, replacement-session rebind, and feature reload now provide restore triggers. Future read-only preview may remain a discrete callback because selecting a historical node should be able to show state transiently without committing a rollback. The same cell that creates Canvas document-ref snapshots restores content plus anchor metadata. C4 therefore lands inside Canvas restore rather than as a second session entry.

Activation **restore** now runs on startup, replacement-session rebind, and feature reload, including Canvas content plus anchor metadata (this is where C4 lands). Reload uses a transient turn-state registry snapshot so an obsolete deferred bootstrap request cannot restore replacement instances or mark them ready. The reload path serializes reload invocations, and the renderer receives `surfaces_changed` only after replacement restoration settles. Later read-only preview can select a past node and resolve its versions into the pane without changing the active branch. The later [pane-and-file-versioning](../docs/design/pane-and-file-versioning.md) rollback action set (conversation-only / pane-only / both) drives `pi.fork` / `pi.navigateTree` (ExtensionCommandContext, available via C1). Selective rollback remains future design over named state cells. = canvas-plan U5–U6. The opt-in user-file store (U7) is its own later plan.

**Boundary.** Owned-pane store only.

---

## Near-term implementation direction

1. Move the `uix.state` delivery seam out of transitional `before_agent_start` flushing and into submit prep before `session.prompt(text)`, after `uix.turn-state` prep appends.
2. _Landed 2026-07-19:_ serialized reload commits current active feature state after restoration settles. It activates replacements and reloads initialized Pi resources. It restores the selected branch through a turn-state registry snapshot, then publishes `surfaces_changed` after restoration settles. Add preview only with the later navigation UI.
3. Introduce the document-engine transaction/event shape for managed documents only when a second concrete writer/listener needs it. The shape covers document kind normalization, exact `sourceId`, and ref-only write events `{ resourceId, kindId, sourceId, beforeSnapshotId, afterSnapshotId, normalized }`.
4. Move canvas wiring toward a first-party default feature boundary: canvas kind, surface writeback handler, agent anchor tools, turn-state contribution, and source-aware listeners live together.
5. Route canvas surface writeback and canvas agent tools through the same document engine once that engine exists. Fold normalization into agent tool results so the model sees final canonical anchored output.

## Reconvene points

- **After C0 + C1:** resume [conversation-render-primitives](../docs/design/conversation-render-primitives.md) (tool-call rendering, the two render registries, the agent-triggered `rich-diff`) on the persisted, extension-backed substrate.
- **C2–C5** are largely independent of the render work and sequenced by dependency above. Pick up by appetite.
