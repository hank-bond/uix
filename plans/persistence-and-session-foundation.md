...
- **`uix.turn-state` pointer:** the turn-state coordinator appends `pi.appendEntry("uix.turn-state", { cwd, state: { [featureId]: <opaque> } })`; the canvas feature's state is `{ "doc://canvas/main": snapshotId }`, but the coordinator never interprets it (see [pane-and-file-versioning](../docs/design/pane-and-file-versioning.md) Log 2026-06-21 for the contribution-keyed/opaque-state model that supersedes the old flat `panes` shape). On resume/navigation, the nearest such entry up the branch says which snapshots were current and which cwd the agent was at; reopen at that cwd (fall back to the home root + notice if the path is gone). Restore granularity is **run boundaries** — matching pi CLI's model — which is why pointers are per-boundary even though the latest file changes continuously. Resolves [pane-and-file-versioning](../docs/design/pane-and-file-versioning.md) open-Q #3 and carries the per-turn cwd (substrate-owned, since the store is path-unaware) that [project-root-vs-agent-cwd](../docs/decisions/2026-06-06-project-root-vs-agent-cwd.md) depends on.
