---
summary: "Build the workspace-file state substrate: auto-initialized git per workspace, session worktrees, turn-boundary checkpoint commits on app-owned refs, checkpoint restore with checkpoint-on-leave, turn-state binding, close-out reclaim, and the diff-review/merge surface."
---
...
Implement the [session worktrees and turn checkpoints decision](../docs/decisions/2026-08-08-session-worktrees-and-turn-checkpoints.md): one git path for workspace files with session worktrees, per-turn checkpoint refs that never touch the work branch, and undo that restores working-tree content from checkpoints. This is the load-bearing piece for the file editor, undo, and concurrent agents.
