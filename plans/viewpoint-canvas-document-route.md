...
- W7 is complete and reviewed. Electron exposes a separate host-only binding bridge with revisioned bootstrap and replacement snapshots. The renderer subscribes before reading, ignores older revisions, and mounts with attachment web roots. Physical roots use `uix-resource://{feature}.viewpoint.{workspace}/{binding}/`, keeping workspace resource and surface-module URLs unchanged. The protocol adapter rejects alternate page directories, acquires a workspace guard, and delegates viewpoint dispatch to the runtime. Playwright launches the real Electron host with production Canvas and Chat plus a test-only Canvas route surface. It verifies rendered CSS, native relative and fragment links, retargeted roots without surface remount, revoked URLs, and window-close revocation. Production Canvas reads remain unchanged. Control snapshots are schema-derived and validated in preload. Subscription cleanup is owned during bootstrap, and Playwright cleanup uses a disposal stack. State and observable names identify their roles, and URL encoding uses the substrate's feature-id grammar. Tests also cover an underscore-named feature without web routes, malformed control data, and disposal during a pending bootstrap read. The shared `FeatureWebRootUrl` brand is validated at its API owner and preserved through the host encoder, roots observable, surface provider, and route client. Type tests reject arbitrary strings and unrelated URL brands. Structural tests cover custom-scheme and HTTP roots, malformed input, and native URL normalization. Verification: `npm run check` passed with 959 tests passing and three skipped. The Playwright test runs inside that check and records a screenshot at `out/test-results/electron-viewpoint.png`.

The [`naming.host-role`](../docs/architecture/conventions/rules/naming.host-role.md) convention governs host-owned names. W7 applies it to the names introduced in this slice, and subsequent units use it for their additions. Existing host-name migration and constrained-string migration are unscheduled entries in the [convention migrations backlog](./backlog.md#convention-migrations). W7 includes the initial `FeatureWebRootUrl` migration as a worked example, not the repository-wide string audit. These migrations are selected independently between plans and are outside this plan's review units.

## Testing choice

Use Playwright for browser and Electron coverage in this plan. Existing Lightpanda tests remain unchanged. Their migration belongs to a separate later plan, not W7 through W9. The W7 test uses Electron's Chromium through Playwright and records screenshots for review without introducing pixel-diff baselines. It launches with hidden windows by default and checks window visibility. Set `UIX_ELECTRON_TEST_SHOW_WINDOW=1` to display the window for debugging.
...
### W7: Carry bindings through the Electron host · **complete and reviewed**
