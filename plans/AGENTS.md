...
- **[session-worktrees-and-turn-checkpoints](./session-worktrees-and-turn-checkpoints.md)** Build the workspace-file state substrate: auto-initialized git per workspace, session worktrees, turn-boundary checkpoint commits on app-owned refs, checkpoint restore with checkpoint-on-leave, turn-state binding, close-out reclaim, and the diff-review/merge surface.
