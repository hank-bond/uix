...
Let Canvas documents compose reusable JavaScript, web components, CSS, HTML, and other browser assets by stable local path. Do not regenerate or embed those assets in every document. The collection belongs to the Canvas feature. The workspace-owned Canvas feature has a conventional `public/` tree and serves it read-only on the same isolated origin as its documents.

This keeps Canvas HTML small, reduces inference tokens, and makes reusable pieces composable across many Canvas writes. A paired persistence control lets a document distinguish hydrated DOM from derived output. Hydrated DOM is meaningful persisted state. Derived output is what the document recreates from persistent source on load.
...
User-created scripts live alongside the Canvas feature, and the user can edit them directly. Copy an npm, GitHub, or other remote source into `vendor/` as a local browser asset. Record its source, version or commit, and license beside it. The runtime fetches or updates nothing automatically.
...
- **Layers.** A component splits into library code, structure, and data. Library code is behavior and styles, referenced by documents and propagating to every instance when changed. Structure is authored markup in the document. Data is props and attributes. The render path generates derived output (rendered diagrams, computed subtrees) from persistent source and never persists it (C3).
