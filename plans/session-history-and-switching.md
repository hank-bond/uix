...
Session switching, reload, and future branch rewind share one state transition lifecycle. New Session is a substrate-provided workspace action, so its invokable action guard reads the workspace's current agent state regardless of whether Chat is installed. While busy, the action skips the backend request; if activity races the renderer guard, main rejects rather than aborting the run. Session switching uses the same no-abort policy: the selector is disabled and skips requests while the agent is running, while main rejects a raced or direct request. Transient notice presentation remains later polish. Reload adopts its own reviewed busy policy when its action path lands rather than inheriting an abort policy speculatively.
...
The same rule applies when opening an older session with no state for a feature, or when you add a feature to a session that predates its turn-state contribution. A new conversation over cloned state is a **fork**, not `new_session` or `Mod+N`: the forked graph retains the selected source path, including its turn-state nodes, and restores from that copied history without inventing a synthetic initial state.
...
Live message taps are a separate, unbuilt contribution path. They react only while messages/turns occur and may update feature working state. Anything from a tap that must survive reload, switch, restart, or rewind must reach durable turn state—inline or through stable referenced ids—by an appropriate lifecycle boundary. Uncommitted buffer state is intentionally ephemeral. Session restoration never reruns taps or regexes historical messages, avoiding duplicate side effects and remaining valid after compaction. The aligned future tap is tracked in [the plans backlog](./backlog.md) and the [UIX-core composition design](../docs/design/uix-core-composition.md).
...
The schema-bound contribution shape gives ordinary TypeScript compile-time agreement between `createSnapshot()` and `restore()`, and runtime contribution validation requires a TypeBox schema plus both functions. The running app does **not** yet execute the planned TS7 source-admission check before loading arbitrary workspace features. Until [feature source admission](../docs/design/feature-source-admission.md) lands, repository typecheck protects in-tree features and runtime validation protects actual snapshot/persisted values, but the loader cannot promise that every arbitrary feature candidate passed the compiler before execution.
