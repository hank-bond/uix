...
The payload-free `new_session` agent request commits current active feature state. It asks `AgentSessionRuntime` for a fresh graph and restores every active cell from `undefined`. It returns the new `SessionSummary` only after restoration settles. Main rejects a raced request while the agent is running. The renderer workspace session controller updates its shared active-session projection from the successful response. Chat invalidates old history and clears/loads against the new id while preserving its draft.
