...
A log of the load-bearing decisions behind Trellis: *why* it exists, *why* it's
shaped as a substrate rather than an app, and *why* the stack is what it is.
...
Every feature decision passes the test: *does this help the pilot see, decide,
or act?* If it's "make the agent smarter," it belongs in pi, not Trellis.
...
still wanted, but as *applications built on Trellis*. The substrate split
...
1. **TUI agents force linear, scroll-heavy interaction.** Questions at the top
   ...
   Substrate consequence: panes must support *inline interactive content*
...
2. **Reviewing agent-generated code is unreviewable at volume.** 1000-line
   ...
   Substrate consequence: extensions need to render *structured artifacts*
...
- **"Report as the conversation, structured."** Rejected in favor of
  *conversation pane is the conversation; reports are artifacts*. Two
