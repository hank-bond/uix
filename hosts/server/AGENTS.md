...
`GET /workspaces/:workspace` and `GET /workspaces/:workspace/sessions/:session` serve the same stateless workspace shell without booting a runtime. A WebSocket upgrade on the workspace-only location lazily boots its one runtime and durably records a fresh Pi session. Its `ready` message returns the accepted session id, canonical path, and private web binding. An upgrade on the canonical location attaches to the named session. The browser replaces its location with that server-authored canonical route and mounts the shared workspace client.
...
Logical `uix-resource://` references remain host-neutral in channel payloads. The browser adapter maps them to workspace-qualified HTTP URLs. Every content request acquires its own workspace guard and dispatches through the runtime's registered content boundary without creating an attachment. Versioned surface modules, CSS, and assets retain exact bytes for the runtime lifetime and use immutable caching. Mutable feature resources default to `no-store`. The HTTP boundary removes runtime-authored cross-origin grants and derives any grant from the configured public origin.

Viewpoint document URLs use `/workspaces/:workspace/viewpoints/:binding/:feature/` as their feature directory. The binding selects the attachment's retained Agent generation through runtime dispatch. The host does not infer an Agent from the workspace or session location. A `web_binding` control message replaces browser feature roots on retarget without replacing the WebSocket or advancing connection recovery. Reconnect supplies a fresh binding before notifying recovery consumers. Old bindings reject new HTTP work, while accepted requests retain independent guards through completion. Complete pages occupy the directory root or one literal segment beneath it, preserving native relative and fragment links without HTML rewriting. Canvas loads its production iframe directly from the bound document route and reloads it on invalidation. Writeback and prompt actions retain the Canvas-owned parent bridge.

The server viewpoint Playwright test launches the built server with production Canvas and Chat plus a test-only route surface. Install its browser with `npx playwright install chromium`. Run the test with `npx vitest run hosts/server/src/viewpoint-web.test.ts`. It writes a review screenshot to `out/test-results/server-viewpoint.png`.
