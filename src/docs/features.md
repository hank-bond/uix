...
`contribute()` returns facet contributions such as resources, channels, feature-namespaced Agent tools, explicit exact-name Agent tools, Agent system-prompt sections, Pi skills, turn state, agent context, and surfaces. Surfaces are optional: a backend-only feature may contribute Agent capabilities or other facets without occupying workspace layout. UIX starts Pi without active built-in tools. The reference app's `workspace_tools` feature independently contributes reason-bearing exact-name `read`, `write`, and `command` plus a passthrough `edit`, while newly scaffolded bare workspaces own editable passthrough `read`, `write`, `edit`, and `bash` source. See [`contributions.md`](./contributions.md), [`channels.md`](./channels.md), [`settings.md`](./settings.md), and [`lifetimes.md`](./lifetimes.md).
