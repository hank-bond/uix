...
- **[features](./features.md)** _(active, reference)_ — Feature entries are trusted local TS/JS modules listed explicitly in uix.workspace.json; each uses defineFeature to default-export a settings-typed FeatureDefinition loaded with jiti, lifetime-scoped under the reload bag, and wired only through @uix/api.
