...
- **[agent](./agent.md)** _(active)_ — Current agent integration in UIX: the cockpit lazily owns an in-memory pi AgentSession, forwards a small event stream to the renderer, supports reload delegation, and binds core canvas read/write tools. Read when working with the current agent driver or canvas tools.
