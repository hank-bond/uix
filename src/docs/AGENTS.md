...
- **[agent](./agent.md)** _(active)_ — How the cockpit drives the agent today: it lazily owns an in-memory pi AgentSession, forwards a small event stream to the renderer, delegates reload, and binds the core anchored document read/write/edit tools.
