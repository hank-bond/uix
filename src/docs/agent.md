...
Chat's connection modal consumes credential descriptors directly: an API method toggles an inline generated form whose empty replacement field identifies stored, environment, or command-backed auth without exposing complete values; source-help popovers clarify that saving writes a literal `auth.json` credential which takes precedence without mutating external sources. Successful persistence refreshes provider/model state and confirms in place, and the explicit `Choose a model` handoff opens the ordinary model search seeded to that backend provider. The search remains editable; UIX does not select a model automatically.
