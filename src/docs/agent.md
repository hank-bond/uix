...
The current canvas agent-tool contribution lives in `src/main/canvas/contributions/agent-tools.ts` and contributes the anchored canvas channel:
...
The canvas state-message contribution factory (`src/main/canvas/contributions/state-messages.ts`) returns two contributions: `uix.pane-visibility` (`{"canvases_open": [...]}`, change-only) and `uix.canvas-diff` (the anchored human-edit hunks, a consuming read computed at the boundary, always sent when present). The substrate `registerStateMessageContributions(...)` helper owns registration and disposal.
