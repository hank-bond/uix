...
A fresh manifest materializes both `settings.agent: {}` and `settings.keybindings: {}` even before values are chosen. This keeps the available configuration surface visible; later selections fill concrete properties. See [`agent.md`](./agent.md) for how model selection and favorites flow through the agent channels. Bound main request handlers can reconcile defaults and atomically replace the complete map through the reserved workspace channel, publishing a confirmed snapshot when it changes. The workspace renderer reconciles active action defaults, retains the main-confirmed map, joins active bindings/conflicts into the action catalog, and keeps inactive persisted ids as unresolved bindings. Direct keyboard dispatch and a public renderer editing API are not yet shipped.
