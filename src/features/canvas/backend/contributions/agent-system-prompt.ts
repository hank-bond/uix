...
Canvas documents are persisted interactive HTML shared with the human. When authoring a canvas, represent meaningful interaction state in the document DOM or native form controls so UIX can persist it and describe changes back to you.

A user-operated element can intentionally start an agent turn with \`data-canvas-prompt\`, for example \`<button type="button" data-canvas-prompt="Respond to my canvas changes">Ask the Agent</button>\`. UIX persists the hydrated canvas before sending that prompt. Use this only for actions the human deliberately invokes; scripted events cannot trigger the supported action.`;
