...
function toParamText(value: unknown): string | undefined {
  ...
  if (typeof value === "string") {
    ...
    if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
      try {
        return toJsonParamText(JSON.parse(trimmed));
      } catch {
        // Incomplete streamed JSON remains readable as its original text.
      }
    }
    return value;
  }
  ...
  try {
    return toJsonParamText(value);
  } catch {
    // Non-serializable values cannot arrive over the channel, but may be local.
    return undefined;
  }
}

function toJsonParamText(value: unknown): string | undefined {
  // This is a display, not JSON to execute. Consume escaped backslashes in
  // pairs so a literal \\n (such as a path or regex) stays literal.
  return JSON.stringify(value, undefined, 2).replace(
    /\\(\\|n|r)/g,
    (escape: string, character: string) =>
      character === "n" ? "\n" : character === "r" ? "\r" : escape,
  );
}
