...
- **[Chat.tsx](./Chat.tsx)** Renders chat transcripts, prompt submission and cancellation, status controls, and provider login.
- **[ChatComposer.tsx](./ChatComposer.tsx)** Owns the high-frequency Chat draft and renders prompt submission and cancellation controls.
- **[composer.ts](./composer.ts)** Derives Chat composer presentation and keyboard intent from shared turn activity.
...
- **[pending-user-identity.ts](./pending-user-identity.ts)** Defines renderer-local pending user-row identities that cannot collide with live or durable transcript ids.
