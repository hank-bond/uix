...
This package holds the host-neutral coordination both concrete hosts compose. An in-memory suite proved the host/runtime boundary. A supervisor maps workspace ids to single-flight boots, workspace handles own one runtime each, and attachments bind connections to session targets with scoped event routing. Concrete Electron and server code never enters this package: `hosts/electron` and `hosts/server` remain the only homes of their platform adapters. The supervisor may import `@uix/runtime` to build handles, but it never imports `@uix/client` or either concrete host.
